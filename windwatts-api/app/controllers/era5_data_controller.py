...
if not _skip_data_init:
    ...
    data_fetcher_router.register_fetcher(
        "athena_ensemble", athena_data_fetcher_ensemble
    )
...
SAMPLE_YEARS = {"s3_era5": [2020, 2021, 2022, 2023]}

# YEARS for which we have era5 data in the S3
ALL_YEARS = {"s3_era5": list(range(2013, 2024))}
...
def validate_height(height: int) -> int:
    if not (0 < height <= 300):
        raise HTTPException(
            status_code=400, detail="Height must be between 1 and 300 meters."
        )
    return height
...
def validate_source(source: str) -> str:
    if source not in VALID_SOURCES:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid source for ERA5 data. Must be one of: {sorted(VALID_SOURCES)}.",
        )
    return source
...
def validate_year(year: int, source: str) -> int:
    if year not in ALL_YEARS[source]:
        raise HTTPException(
            status_code=400,
            detail="Invalid year for ERA5 data. Currently supporting years 2013-2023",
        )
    return year
...
def validate_n_neighbor(n_neighbor: int) -> int:
    if not 1 <= n_neighbor <= 4:  # have to change the limit if needed later on
        raise HTTPException(
            status_code=400,
            detail="Invalid number of neighbors. Currently supporting upto 4 nearest neighbors",
        )
    return n_neighbor


def _get_windspeed_core(
    lat: float, lng: float, height: int, avg_type: str, source: str
):
    """
    ...
    params = {"lat": lat, "lng": lng, "height": height, "period": avg_type}
...
def get_windspeed_with_avg_type(
    ...
    ensemble: bool = Query(
        False, description="If true, use ensemble model (athena_ensemble)."
    ),
    source: str = Query(DEFAULT_SOURCE, description="Source of the data."),
):
    try:
        if ensemble:
            return _get_windspeed_core(
                lat, lng, height, avg_type, source="athena_ensemble"
            )
        else:
...
def get_windspeed(
    ...
    try:
        if ensemble:
            return _get_windspeed_core(
                lat, lng, height, period, source="athena_ensemble"
            )
        else:
...
def _get_energy_production_core(
    lat: float, lng: float, height: int, powercurve: str, period: str, source: str
):
    """
    ...
    try:
        if ensemble:
            return _get_energy_production_core(
                lat, lng, height, powercurve, period, source="athena_ensemble"
            )
        else:
            return _get_energy_production_core(
...
def download_timeseries_csv(
    gridIndex: str = Query(
        ..., description="Grid index with respect to user selected coordinate"
    ),
    years: List[int] = Query(
        SAMPLE_YEARS["s3_era5"], description="years of which the data to download"
    ),
...
def download_timeseries_csv_batch(
    ...
    try:
        ...
        spooled = tempfile.SpooledTemporaryFile(
            max_size=30 * 1024 * 1024, mode="w+b"
        )  # 30MB threshold (Each decompressed file is around 5.3 MB)
