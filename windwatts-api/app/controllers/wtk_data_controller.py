...
SAMPLE_YEARS = {"s3_wtk": [2018, 2019, 2020]}
...
def validate_source(source: str) -> str:
    if source not in VALID_SOURCES:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid source for WTK data. Must be one of: {sorted(VALID_SOURCES)}.",
        )
    return source
...
def validate_year(year: int, source: str) -> int:
    if year not in ALL_YEARS[source]:
        raise HTTPException(
            status_code=400,
            detail="Invalid year for WTK data. Currently supporting years 2000-2022",
        )
    return year
...
def download_csv(
    ...
    years: List[int] = Query(
        SAMPLE_YEARS["s3_wtk"], description="years of which the data to download"
    ),
...
def nearest_locations(
    ...
    n_neighbors: int = Query(
        1, description="Number of nearest grid points.", ge=1, le=4
    ),
    source: str = Query(DEFAULT_SOURCE, description="Source of the data"),
):
    try:
        ...
        result = fetcher.find_nearest_locations(
            lat=lat, lng=lng, n_neighbors=n_neighbors
        )
