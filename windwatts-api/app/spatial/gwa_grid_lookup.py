...
class GWAGridLookup(AbstractSpatialLookup):
    """Nearest land-cell lookup on a masked raster grid (GWA, Global Wind Atlas - North America)."""
    ...
    def find_nearest(
        self, lat: float, lng: float, max_search_cells: int = 4
    ) -> GridPoint:
        row, col = self.lat_lon_to_row_col(lat, lng)
        lands = self._find_existing_neighbors(row, col, 1, max_search_cells)
        if not lands:
            raise NoLandCellError(
                f"No land cell within {max_search_cells} cells of ({lat}, {lng})"
            )
        r, c = lands[0]
    ...
    def _find_existing_neighbors(
        self, row: int, col: int, n: int, max_search_cells: int
    ) -> list[tuple[int, int]]:
        candidates = []
